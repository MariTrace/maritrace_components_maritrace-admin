...
const ArchivedUserManagementPage: React.FC<Props> =({parentUuid = ""}) => {
    ...
    return (
        <div id="archived_user_management_page" className="page_container">
            <h1 className="page_title">Archived Users</h1>
                ...
                <Table striped bordered hover>
                    <thead>
                        <tr>
                            ...
                            <th className="sortable_column" onClick={() => toggleSort('last_login')}>Last Login {sortColumn.name == "last_login" ? sortColumn.dir ? <span className="sorting_arrow">&darr;</span> : <span className="sorting_arrow">&uarr;</span> : null}</th>
                            <th className="sortable_column" onClick={() => toggleSort('date_expires')}>Expires{sortColumn.name == "date_expires" ? sortColumn.dir ? <span className="sorting_arrow">&darr;</span> : <span className="sorting_arrow">&uarr;</span> : null}</th>
                    ...
                    {userAccountsList?.map((item, index) => (
                        <tr key={item.user_account_uuid}>
                            ...
                            <td>{G.displayValue(item.date_removed ? item.date_removed.split("T")[0] : item.date_expires ? item.date_expires.split("T")[0] : ""  )}</td>
                            <td>{item.removed_full_name}</td>
                            <td>
                                <Button className="btn btn-light fas pt1-5 action_buttons2" onClick={() => {AM.handleArchiving([item.user_account_uuid], false, true)}}>Un-archive</Button>
                            </td>
                        </tr>
                    ))}
...
export default ArchivedUserManagementPage;